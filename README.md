...
CephTower 是一个用于支持管理 Ceph 集群的 Web 控制台项目，后端基于 Go，前端基于 React 与 Ant Design，通过 Ceph Manager Dashboard API 管理整个 Ceph 集群。
...
- 使用 React、Ant Design、Vite、TypeScript 初始化管理控制台
- 首屏为集群概览工作台，不做营销型落地页
...
│   └── go.mod                    # Go 模块配置
├── frontend/                     # React + Ant Design + Vite Web 控制台
│   ├── src/
│   │   ├── api/                  # 前端 API 调用
│   │   ├── App.tsx               # 应用主界面
│   │   ├── main.tsx              # 前端入口
...
| Ceph 服务层 | `backend/internal/service` | 编排 Ceph 数据同步、调度和数据库适配 |
...
| 模块 | 路径 | 功能描述 |
|------|------|----------|
| 应用入口 | `frontend/src/main.tsx` | 初始化 React 应用 |
| 主界面 | `frontend/src/App.tsx` | 集群管理控制台首页 |

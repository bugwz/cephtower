...
func (h *Handler) ProxyPath(w http.ResponseWriter, r *http.Request) {
	routePath := strings.TrimPrefix(r.Pattern, r.Method+" /api/v1")
	...
	h.Proxy(r.Method, cephPath)(w, r)
}

func (h *Handler) Proxy(method string, cephPath string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		payload, err := h.ceph.Raw(r.Context(), method, path, request.Query, request.Body)
		if err != nil {
...
type DashboardProxyRequest = RawJSONRequest
type DashboardProxyResponse = RawJSONResponse

...
func buildMutationRequestContracts() map[string]RequestContract {
	...
	add([]string{"rgw_zone.update"}, true, map[string]JSONField{"read_only": boolField(false), "tier_type": stringField(false, "", "archive"), "master": boolField(false), "default": boolField(false), "sync_from_all": boolField(false), "sync_from": stringField(false), "endpoints": stringField(false), "name": stringField(true), "new_name": stringField(true), "zonegroup": stringField(false), "realm_id": stringField(false)})

...
func buildMutationRequestContracts() map[string]RequestContract {
	...
	add([]string{"ceph_user.delete"}, true, map[string]JSONField{"entity": stringField(true)})
	add([]string{"ceph_user.import"}, true, map[string]JSONField{"keyring": {Type: "string", Required: true, WriteOnly: true}})
	add([]string{"health.mute"}, false, map[string]JSONField{"ttl": stringField(false), "sticky": boolField(false)})
	...
	add([]string{"rbd_image.create"}, true, map[string]JSONField{"image_spec": stringField(true), "size": integerField(true), "data_pool": stringField(false), "object_size": integerField(false), "stripe_unit": integerField(false), "stripe_count": integerField(false)})
	add([]string{"rbd_image.update"}, true, map[string]JSONField{"size": integerField(false), "allow_shrink": boolField(false), "name": stringField(false), "feature_action": stringField(false, "enable", "disable"), "features": stringField(false)})
	add([]string{"rbd_image.action"}, true, map[string]JSONField{"action": stringField(true, "config-set", "config-remove", "snapshot-purge", "feature-enable", "feature-disable", "flatten", "sparsify", "copy", "deep-copy", "rename", "move-to-trash", "mirror-enable-journal", "mirror-enable-snapshot", "mirror-disable", "mirror-promote", "mirror-demote", "mirror-resync", "mirror-snapshot"), "config_name": stringField(false), "config_value": stringField(false), "feature": stringField(false, "exclusive-lock", "object-map", "fast-diff", "journaling", "deep-flatten"), "destination": stringField(false), "expires_at": stringField(false)})
	...
	add([]string{"rbd_group.snapshot"}, true, map[string]JSONField{"group_spec": stringField(true), "name": stringField(true), "new_name": stringField(false), "action": stringField(true, "create", "remove", "rollback", "rename")})
	add([]string{"rbd_group.action"}, true, map[string]JSONField{"group_spec": stringField(true), "action": stringField(true, "rename", "remove"), "name": stringField(false)})
	add([]string{"rbd_group.member"}, true, map[string]JSONField{"group_spec": stringField(true), "image": stringField(true), "action": stringField(true, "add", "remove")})
	...
	add([]string{"rbd_trash.purge"}, true, map[string]JSONField{"pool": stringField(true), "expired_before": stringField(false)})
	...
	add([]string{"rgw_user.caps"}, true, map[string]JSONField{"uid": stringField(true), "action": stringField(true, "add", "rm"), "type": stringField(true), "permission": stringField(true, "read", "write", "read,write", "*")})
	add([]string{"rgw_user.quota"}, true, map[string]JSONField{"uid": stringField(true), "scope": stringField(true, "user", "bucket"), "enabled": boolField(true), "max_size": integerField(true), "max_objects": integerField(true)})
	add([]string{"rgw_account.quota"}, true, map[string]JSONField{"account_id": stringField(true), "scope": stringField(true, "account", "bucket"), "enabled": boolField(true), "max_size": integerField(true), "max_objects": integerField(true)})
	add([]string{"rgw_account.update"}, true, map[string]JSONField{"account_id": stringField(true), "account_name": stringField(false), "email": stringField(false), "max_users": integerField(false), "max_roles": integerField(false), "max_groups": integerField(false), "max_buckets": integerField(false), "max_access_keys": integerField(false)})
	add([]string{"rgw_account.delete"}, true, map[string]JSONField{"account_id": stringField(true)})
	...
	add([]string{"rgw_zone.update"}, true, map[string]JSONField{"access_key": optionalSecret, "secret_key": optionalSecret, "read_only": boolField(false), "tier_type": stringField(false, "", "archive"), "master": boolField(false), "default": boolField(false), "sync_from_all": boolField(false), "sync_from": stringField(false), "endpoints": stringField(false), "name": stringField(true), "new_name": stringField(true), "zonegroup": stringField(false), "realm_id": stringField(false)})

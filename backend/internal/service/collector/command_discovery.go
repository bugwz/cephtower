...
type ClusterDiscoverer func(context.Context, *store.Database, *store.CephCluster) error
...
func DiscoverAndSyncCephCluster(ctx context.Context, db *store.Database, cluster *store.CephCluster) error {
	return DiscoverAndSyncCephClusterWithWorkDir(ctx, db, cluster, "")
}

func DiscoverAndSyncCephClusterWithWorkDir(ctx context.Context, db *store.Database, cluster *store.CephCluster, workDir string) error {
	if cluster == nil {
		return nil
	}
...
func SyncCephRuntimeFiles(ctx context.Context, db *store.Database, workDir string) error {
...
func saveDiscoveredHosts(ctx context.Context, db *store.Database, clusterID uint, load func() ([]ceph.Host, error)) {
	hosts, err := load()
	if err != nil {
		return
	}
...
func saveDiscoveredOSDs(ctx context.Context, db *store.Database, clusterID uint, load func() ([]map[string]any, error)) {
	osds, err := load()
	if err != nil {
		return
	}
...
func saveDiscoveredOSDFlags(ctx context.Context, db *store.Database, clusterID uint, load func() ([]string, error)) {
	flags, err := load()
	if err != nil {
		return
	}
...
func saveDiscoveredDaemons(ctx context.Context, db *store.Database, clusterID uint, load func() ([]map[string]any, error)) {
	daemons, err := load()
	if err != nil {
		return
	}
...
func saveDiscoveredServices(ctx context.Context, db *store.Database, clusterID uint, load func() ([]map[string]any, error)) {
	services, err := load()
	if err != nil {
		return
	}
...
func saveDiscoveredMons(ctx context.Context, db *store.Database, clusterID uint, load func() (map[string]any, error)) {
	monitor, err := load()
	if err != nil {
		return
	}
...
func saveDiscoveredMgrs(ctx context.Context, db *store.Database, clusterID uint, load func() (map[string]any, error)) {
	mgrDump, err := load()
	if err != nil {
		return
	}
...
func saveDiscoveredMDSs(ctx context.Context, db *store.Database, clusterID uint, load func() (map[string]any, error)) {
	fsDump, err := load()
	if err != nil {
		return
	}
...
func saveDiscoveredMgrModules(ctx context.Context, db *store.Database, clusterID uint, load func() (map[string]any, error)) {
	modules, err := load()
	if err != nil {
		return
	}
...
func saveDiscoveredConfiguration(ctx context.Context, db *store.Database, clusterID uint, load func() ([]map[string]any, error)) {
...
func saveDiscoveredSummary(ctx context.Context, db *store.Database, clusterID uint, payload json.RawMessage) error {
	...
	return db.UpsertClusterRecord(ctx, clusterID, map[string]any{}, &record)
}

func saveDiscoveredHealthChecks(ctx context.Context, db *store.Database, clusterID uint, payload json.RawMessage) int {
...
func saveDiscoveredPools(ctx context.Context, db *store.Database, clusterID uint, pools []map[string]any) int {
	now := time.Now()
...
func saveDiscoveredRBDImages(ctx context.Context, db *store.Database, clusterID uint, images []map[string]any) int {
	now := time.Now()
...
func saveDiscoveredFilesystems(ctx context.Context, db *store.Database, clusterID uint, filesystems []map[string]any) int {
	now := time.Now()
...
func saveDiscoveredRGWDaemons(ctx context.Context, db *store.Database, clusterID uint, daemons []map[string]any) int {
	now := time.Now()
...
func saveDiscoveredRGWUsers(ctx context.Context, db *store.Database, clusterID uint, users []map[string]any) int {
	now := time.Now()
...
func saveDiscoveredRGWBuckets(ctx context.Context, db *store.Database, clusterID uint, buckets []map[string]any) int {
	now := time.Now()
...
func replaceDiscoveredRecords[T any](ctx context.Context, db *store.Database, clusterID uint, model any, records []T) {
	if len(records) == 0 {
		_ = db.ReplaceClusterRecords(ctx, clusterID, model, []T{})
		return
	}
	_ = db.ReplaceClusterRecords(ctx, clusterID, model, &records)
}

...
type databaseCephClient struct {
	...
	workDir    string
	submitTask func(string, func(context.Context) error) error
	collector  *Service
}

// DatabaseCephClient is exported for the cephproxy facade. Callers should use
// service/cephproxy rather than constructing this cache-backed client directly.
type DatabaseCephClient struct{ *databaseCephClient }

func (c *DatabaseCephClient) UseCollector(service *Service) *DatabaseCephClient {
	c.databaseCephClient.collector = service
	return c
}

func NewDatabaseCephClient(database func() *store.Database, workDirs ...string) *DatabaseCephClient {
	workDir := ""
	if len(workDirs) > 0 {
		workDir = workDirs[0]
	}
	return &DatabaseCephClient{databaseCephClient: &databaseCephClient{database: database, workDir: workDir}}
}

func NewDatabaseCephClientWithSubmitter(
	database func() *store.Database,
	submitTask func(string, func(context.Context) error) error,
	workDirs ...string,
) *DatabaseCephClient {
	client := NewDatabaseCephClient(database, workDirs...)
	client.submitTask = submitTask
	return client
}

func (c *databaseCephClient) dashboardClient(ctx context.Context) (*dashboard.DashboardClient, error) {
	cluster, err := c.database().FirstCluster(ctx)
	if err != nil {
		return nil, err
	}
...
func (c *databaseCephClient) ClusterSummary(ctx context.Context) (ceph.ClusterSummary, error) {
	...
	err = c.database().FindClusterRecord(ctx, clusterID, nil, &record)
...
func (c *databaseCephClient) ListHosts(ctx context.Context, options ceph.ListHostsOptions) ([]ceph.Host, error) {
	...
	if err := c.database().ListClusterRecords(ctx, clusterID, "hostname asc", &records); err != nil {
		return nil, err
	}
...
func (c *databaseCephClient) ListOSDs(ctx context.Context, options ceph.ListOSDsOptions) ([]map[string]any, error) {
	...
	if err := c.database().ListClusterRecords(ctx, clusterID, "osd_id asc", &records); err != nil {
		return nil, err
	}
...
func (c *databaseCephClient) OSDFlags(ctx context.Context) ([]string, error) {
	...
	if err := c.database().ListClusterRecords(ctx, clusterID, "name asc", &records); err != nil {
		return nil, err
	}
...
func (c *databaseCephClient) localHealth(ctx context.Context) (map[string]any, error) {
	...
	err = c.database().FindClusterRecord(ctx, clusterID, nil, &summary)
...
func (c *databaseCephClient) localServicesRaw(ctx context.Context) (json.RawMessage, error) {
	...
	if err := c.database().ListClusterRecords(ctx, clusterID, "service_name asc", &records); err != nil {
		return nil, err
	}
...
func (c *databaseCephClient) localPoolsRaw(ctx context.Context) (json.RawMessage, error) {
	...
	if err := c.database().ListClusterRecords(ctx, clusterID, "pool_name asc", &records); err != nil {
		return nil, err
	}
	items := make([]map[string]any, 0, len(records))
	for _, record := range records {
...
func (c *databaseCephClient) localRBDImagesRaw(ctx context.Context, trash bool) (json.RawMessage, error) {
	...
	if err := c.database().ListClusterRecordsFiltered(ctx, clusterID, map[string]any{"trash": trash}, "image_spec asc", &records); err != nil {
		return nil, err
	}
	items := make([]map[string]any, 0, len(records))
	for _, record := range records {
...
func (c *databaseCephClient) localRGWDaemonsRaw(ctx context.Context) (json.RawMessage, error) {
	...
	if err := c.database().ListClusterRecords(ctx, clusterID, "service_id asc", &records); err != nil {
		return nil, err
	}
	items := make([]map[string]any, 0, len(records))
	for _, record := range records {
...
func (c *databaseCephClient) localRGWDaemonRaw(ctx context.Context, serviceID string) (json.RawMessage, error) {
	...
	err = c.database().FindClusterRecord(ctx, clusterID, map[string]any{"service_id": serviceID}, &record)
	if errors.Is(err, store.ErrRecordNotFound) {
...
func (c *databaseCephClient) localRGWUsersRaw(ctx context.Context) (json.RawMessage, error) {
	...
	if err := c.database().ListClusterRecords(ctx, clusterID, "uid asc", &records); err != nil {
		return nil, err
	}
	items := make([]map[string]any, 0, len(records))
	for _, record := range records {
...
func (c *databaseCephClient) localRGWUserRaw(ctx context.Context, uid string) (json.RawMessage, error) {
	...
	err = c.database().FindClusterRecord(ctx, clusterID, map[string]any{"uid": uid}, &record)
	if errors.Is(err, store.ErrRecordNotFound) {
		return json.Marshal(map[string]any{})
	}
	if err != nil {
...
func (c *databaseCephClient) localRGWBucketsRaw(ctx context.Context) (json.RawMessage, error) {
	...
	if err := c.database().ListClusterRecords(ctx, clusterID, "bucket asc", &records); err != nil {
		return nil, err
	}
	items := make([]map[string]any, 0, len(records))
	for _, record := range records {
...
func (c *databaseCephClient) localRGWBucketRaw(ctx context.Context, bucket string) (json.RawMessage, error) {
	...
	err = c.database().FindClusterRecord(ctx, clusterID, map[string]any{"bucket": bucket}, &record)
	if errors.Is(err, store.ErrRecordNotFound) {
		return json.Marshal(map[string]any{})
	}
	if err != nil {
...
func (c *databaseCephClient) localConfigurationItemRaw(ctx context.Context, name string) (json.RawMessage, error) {
	...
	err = c.database().FindClusterRecord(ctx, clusterID, map[string]any{"name": name}, &record)
	if errors.Is(err, store.ErrRecordNotFound) {
		return json.Marshal(map[string]any{})
	}
	if err != nil {
...
func (c *databaseCephClient) fetchModule(ctx context.Context, module string) {
	...
	taskName := fmt.Sprintf("refresh-ceph-data:%d:%s", clusterID, module)
	_ = c.submitTask(taskName, func(taskCtx context.Context) error {
		var err error
		if c.collector != nil {
			_, err = c.collector.RefreshModule(taskCtx, clusterID, module)
		} else {
			_, err = RunDataFetchModule(taskCtx, c.database, clusterID, module)
		}
		return err
	})
}
...
func (c *databaseCephClient) fetchModulesForPath(ctx context.Context, path string) {
	...
	case strings.HasPrefix(path, "/api/settings"):
		c.fetchModule(ctx, fetchModuleSettings)
	case strings.HasPrefix(path, "/api/feature_toggles"):
		c.fetchModule(ctx, fetchModuleFeatureToggles)
	case strings.HasPrefix(path, "/api/mgr/module"):
		c.fetchModule(ctx, fetchModuleMgrModules)
		c.fetchModule(ctx, fetchModuleFeatureToggles)
	case strings.HasPrefix(path, "/api/grafana"),
		strings.HasPrefix(path, "/api/prometheus"),
		strings.HasPrefix(path, "/api/telemetry"),
		strings.HasPrefix(path, "/api/feedback"):
		c.fetchModule(ctx, fetchModuleSettings)
		c.fetchModule(ctx, fetchModuleIntegrationStatus)
	}
}

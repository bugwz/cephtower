...
func TestDatabaseCephClientUsesLocalClusterSummary(t *testing.T) {
	...
	if err := db.Insert(context.Background(), &store.CephClusterSummary{
	...
	client := NewDatabaseCephClient(func() *store.Database { return db })
...
func TestDatabaseCephClientLocalRawDoesNotStubUncachedDashboardDetails(t *testing.T) {
	db := openDatabaseCephClientTestDB(t)
	client := NewDatabaseCephClient(func() *store.Database { return db })
	paths := []string{
	...
	for _, path := range paths {
		t.Run(path, func(t *testing.T) {
			payload, ok, err := client.localRaw(context.Background(), path)
			if err != nil {
				t.Fatalf("localRaw() returned error: %v", err)
			}
			if ok {
				t.Fatalf("localRaw() = ok with payload %s, want pass-through to Ceph dashboard", payload)
			}
		})
	}
}
...
func openDatabaseCephClientTestDB(t *testing.T) *store.Database {
	t.Helper()

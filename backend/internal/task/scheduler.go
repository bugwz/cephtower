...
type ScheduledTask func(context.Context)
...
func DailyAt(hour int) Schedule {
	return func(now time.Time) time.Time {
...
func (s *Scheduler) Register(name string, firstAt time.Time, schedule Schedule, task ScheduledTask) error {
	if name == "" {
